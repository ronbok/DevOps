<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to create the Tenant Manifest Tables used by the SaaS
    appliction Steward AppSuite using an Azure AD credential with proper RBAC permissions to create such a group.
    This PowerShell Script makes it easier to create the Tenant Manifest Tables instead of using the Management Portal interactively.
.DESCRIPTION 
    This creates all four (4) required Steward AppSuite Tenant Manifest tables.  The table names are predefined.

.NOTES 
    File Name  : Create-TenantManifestTables.ps1
               :
    ...
    Get-Help ./Create-TenantManifestTables.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureSAName
    Example: "sassubcom1"
.EXAMPLE
    ./Create-TenantManifestTables.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Subscription_Common" -AzureSAName "sassubcom1" -Verbose
.EXAMPLE
    ./Create-TenantManifestTables.ps1 "AppSuite" "East US 2" "Subscription_Common" "sassubcom1" -Verbose
.INPUTS
    The [Subscription] parameter is the name of the Azure subscription.

    The [AzureRegion] parameter is the name of the Azure Region/Location in which the Storage Account exists to place the table.
...
Param(
    ...
    [string]$AzureRegion,
    [Parameter(Mandatory=$True, Position=2, HelpMessage='The name of the Azure Resource Group where the Tenant Manifest Table is stored.')]
    [string]$AzureRGName,
...
$Table = New-AzureStorageTable -Name "TMDevelopment" -Context $SaC
$Table = New-AzureStorageTable -Name "TMTest" -Context $SaC
$Table = New-AzureStorageTable -Name "TMStage" -Context $SaC
$Table = New-AzureStorageTable -Name "TMProduction" -Context $SaC
