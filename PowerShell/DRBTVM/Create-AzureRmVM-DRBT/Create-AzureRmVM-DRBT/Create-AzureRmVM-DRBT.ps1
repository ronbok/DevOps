<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to create a new RH Linux DataRobot VM using an
	Azure AD credential with proper RBAC permissions.  This PowerShell Script makes it easier
    to create a new RH Linux DataRobot VM instead of using the Management Portal interactively.
.DESCRIPTION 
    This creates one RH Linux DataRobot VM from the command line arguments passed into it.
...
.NOTES 
    File Name  : Create-AzureRmVM-DRBT.ps1
               :
    ...
    Optional   : This can be called from a .bat file as desired to create one or more Azure Resource Groups.
			   : An example is included which demonstrates how to provide input for a RH Linux DataRobot VM
			   : within a Resource Group.
               :
    Created    : 06/08/2016 v1.0 Derived from Create-AzureRmVM-SSAS.ps1
    Update     : 06/11/2016 v1.1 Updated with proper -tag parameters for $PIP.
    ...
    Get-Help ./Create-AzureRmVM-DRBT.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.EXAMPLE
    ./Create-AzureRmVM-DRBT.ps1 Create-AzureRmVM-SSAS.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGEnvironmentName "Development" -AzureRGTenantName "Common" -VNetName "AppSuite" -VMName "SSAS-DEV01" -PrivateIPAddress "10.125.128.132" -VMSize "Standard_A3" -AzureSAName "sasdcs" -AzureSAType "Standard_GRS" -Verbose
.EXAMPLE
    ./Create-AzureRmVM-DRBT.ps1 "AppSuite" "East US 2" "Development" "Common" "AppSuite" "SSAS-DEV01" "10.125.128.132" "Standard_A3" "sasdcs" "Standard_GRS" -Verbose
.INPUTS
...
[string]$VMName = "DRBT-A-COM01"
[string]$PrivateIPAddress = "10.125.132.101"
[string]$VMSize = "Standard_D12"
...
If ($CheckDNS)
{
	...
	# For devices in Subscription_Common, if they are on the management network the subnet must be excepted from the naming convention:
    If ($($AzureRGEnvironmentName + "_" + $AzureRGTenantName) -eq "Subscription_Common")
    {
    ...
    $AVSet = New-AzureRmAvailabilitySet -Location $AzureRegion –Name $($AzureRGEnvironmentName + "_" + $($VMName.Split("-")[0] + "_" + $VMName.Split("-")[1])) –ResourceGroupName $($AzureRGEnvironmentName + "_" + $AzureRGTenantName) -ErrorAction SilentlyContinue
    ...
    Set-AzureRmVMOperatingSystem -VM $NewVM -Linux -ComputerName $VMName -Credential $Credentials
    Set-AzureRmVMSourceImage -VM $NewVM -PublisherName "Redhat" -Offer "RHEL" -Skus "7.2" -Version "latest"
    ...
    Add-DataDisk -ResourceGroup $($AzureRGEnvironmentName + "_" + $AzureRGTenantName) -VMName $VMName -DiskSize 200 -LUN 1 -DiskLabel "Data" -Uri $SA.PrimaryEndpoints.Blob.ToString()
