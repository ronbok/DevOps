<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to delete one or more Service Bus Namespace(s) using an
	Azure AD credential.  This PowerShell Script makes it easier to delete one or more Service Bus Namespace(s) instead
...
.DESCRIPTION 
    This PowerShell Script makes it easier delete one or more Azure Service Bus Namespace(s) than using the Azure
    Management Console.
 
.NOTES 
    File Name  : Delete-AzureSBNamespace.ps1
    ...
    Requires   : Azure Subscription needs to be setup in advance and you must know the name.
               ...
               : User must have sufficient privledges to delete an existing Service Bus Namespace within the specified subscription.
               :
    Optional   : This can be called from a .bat file as desired to delete one or more Service Bus Namespace(s).
			   : An example is included which demonstrates how to provide input to delete one or more Service Bus Namespace(s).
               :
    Created    : 03/31/2016
	Updated	   : 03/31/2016 v1.0
	Updated    : MM/DD/YYYY
    ...
    Get-Help ./Delete-AzureSBNamespace.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureSBNameSpace
    Example: aspen1devsb
.EXAMPLE
    ./Delete-AzureSBNamespace.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureSBNameSpace "sassbdcm,sassbtcm,sassbscm,sassbpcm" -Verbose
.EXAMPLE
    ./Delete-AzureSBNamespace.ps1 "AppSuite" "East US 2" "sassbdcm,sassbtcm,sassbscm,sassbpcm" -Verbose
.INPUTS
...
Param(
    ...
    [string]$AzureRegion,
    [Parameter(Mandatory=$True, Position=2, HelpMessage='The Azure Service Bus Namespace.')]
...
#region Delete-SBNamespace()

Function Delete-SBNamespace()
{

    Param  ([string]$Subscription, [string]$AzureLocation, [string]$SBNameSpace)

    # Console output
    Write-Verbose -Message "[Information] Azure Service Bus Namespace $SBNameSpace exists in $Subscription."
    Write-Verbose -Message " "
    ...
    Try
    {
        ...
        # Console output
        Write-Verbose -Message "[Start] Deleting existing Azure Service Bus Namepsace: $SBNameSpace in $Subscription."
        Write-Verbose -Message " "

        # Create Azure Service Bus NameSpace.
        $AzureSBNS = Remove-AzureSBNamespace -Name $SBNameSpace -Force -Confirm:$False

        # Console output
        Write-Verbose -Message "[Finish] Deleted existing Azure Service Bus Namepsace: $SBNameSpace, in $AzureLocation."
        Write-Verbose -Message " "
...
While ($Count -le $AzureSBNameSpaces.Count-1)
    ...
    If (![string]::IsNullOrEmpty($CheckSB))
        ...
        # Call Function
        $AzureSBNS = Delete-SBNamespace $Subscription $AzureRegion $($AzureSBNameSpaces[$Count])
