...
.SYNOPSIS 
    ...
    already exists by the name AppSuiteKeyVaultProduction and the following code uses these values of ClientID/Key from that
    specific application instance.
.DESCRIPTION 
    This grants Azure Key Vault permissions to an Azure AD Application previously registered.

.NOTES 
    File Name  : Grant-AppSuiteKeyVaultProduction.ps1
               :
    ...
    Get-Help ./Grant-AppSuiteKeyVaultProduction.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureKeyVaultName
    Example: "sasscs"
.EXAMPLE
    ./Grant-AppSuiteKeyVaultProduction.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Production_Common" -AzureKeyVaultName "saspcs" -Verbose
.EXAMPLE
    ./Grant-AppSuiteKeyVaultProduction.ps1 "AppSuite" "East US 2" "Production_Common" "saspcs" -Verbose
.INPUTS
...
# Azure AD AppSuiteKeyVaultProd ClientID/Key as stored in KeePass.  These values should probably end up in Key Vault itself instead.  Backlog task.
$AADClientID = "cb25233b-b898-4bc7-af21-35f9e37a1582"
$AADKey = "v/C7sriD9ZmujluUIPOtGnufUUBFKa98V3RQuOsRuIo="
...
{
    $Found = $KV.AccessPolicies[$Count].DisplayName -like "AppSuiteKeyVaultProd*"
    If ($Found)
    {
...
If ($KV.EnabledForDiskEncryption)
{
    ...
    # Grant AppSuiteKeyVaultProduction access with full control to manage Key Vault.
