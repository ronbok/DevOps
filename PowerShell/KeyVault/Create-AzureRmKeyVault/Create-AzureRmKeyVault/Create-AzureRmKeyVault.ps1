<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to create one or more Azure Key Vaults using an
	Azure AD credential with proper RBAC permissions to create such a vault.  This PowerShell Script makes it easier
    to create one or more Azure Key Vaults instead of using the Management Portal interactively.
    Though at present, no UI exists.
.DESCRIPTION 
    This creates one or more Azure Key Vaults.

.NOTES 
    File Name  : Create-AzureRmKeyVault.ps1
               :
    ...
    Get-Help ./Create-AzureRmKeyVault.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.EXAMPLE
    ./Create-AzureRmKeyVault.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Development_Common,Test_Common,Stage_Common,Production_Common" -AzureKeyVaultName "sasdcs,sastcs,sasscs,saspcs" -Verbose
.EXAMPLE
    ./Create-AzureRmKeyVault.ps1 "AppSuite" "East US 2" "Development_Common,Test_Common,Stage_Common,Production_Common" "sasdcs,sastcs,sasscs,saspcs" -Verbose
.INPUTS
    ...
    While ($Count -le $AzureRGNames.Count-1)
    {
        ...
        If ([string]::IsNullOrEmpty($CheckKV))
        {
            ...
            Try
            {
	            ...
	            # Console output with -Verbose only
                Write-Verbose -Message "[Start] Attempting to create Azure Key Vault: $($AzureKeyVaultNames[$Count])."
                Write-Verbose -Message ""

                # Create new Key Vault
                $KeyVault = New-AzureRmKeyVault -VaultName $AzureKeyVaultNames[$Count] -ResourceGroupName $AzureRGNames[$Count] -Location $AzureRegion -Sku standard -Tags @{Name="Environment";Value=$($AzureRGNames[$Count].Split('_')[0])}, @{Name="Tenant";Value=$($AzureRGNames[$Count].Split('_')[1])} -EnabledForDeployment -EnabledForTemplateDeployment -EnabledForDiskEncryption
                
                # Setup logging on the Key Vault
                Set-AzureRmDiagnosticSetting -ResourceId $KeyVault.ResourceId -StorageAccountId $SA.Id -Enabled $True -Categories AuditEvent
                               
                # Console output with -Verbose only
                Write-Verbose -Message "[Finish] Created Azure Key Vault: $($AzureKeyVaultNames[$Count])."
                Write-Verbose -Message ""
            ...
            Catch
            {
	            ...
	            Write-Verbose -Message "[Error:$PSScriptName] Attempt to create Azure Key Vault: $($AzureKeyVaultNames[$Count]) failed."
                Write-Verbose -Message ""
        ...
        Else
        {
            ...
            # Console output with -Verbose only
            Write-Verbose -Message "[Error:$PSScriptName] Attempt to create Azure Key Vault: $($AzureKeyVaultNames[$Count]) failed.  Duplicate name."
            Write-Verbose -Message ""
