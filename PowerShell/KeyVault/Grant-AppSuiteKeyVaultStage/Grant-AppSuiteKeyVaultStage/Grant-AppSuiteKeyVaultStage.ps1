...
.SYNOPSIS 
    ...
    already exists by the name AppSuiteKeyVaultStage and the following code uses these values of ClientID/Key from that
    specific application instance.
.DESCRIPTION 
    This grants Azure Key Vault permissions to an Azure AD Application previously registered.

.NOTES 
    File Name  : Grant-AppSuiteKeyVaultStage.ps1
               :
    ...
    Get-Help ./Grant-AppSuiteKeyVaultStage.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureKeyVaultName
    Example: "sasscs"
.EXAMPLE
    ./Grant-AppSuiteKeyVaultStage.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Stage_Common" -AzureKeyVaultName "sasscs" -Verbose
.EXAMPLE
    ./Grant-AppSuiteKeyVaultStage.ps1 "AppSuite" "East US 2" "Stage_Common" "sasscs" -Verbose
.INPUTS
...
# Azure AD AppSuiteKeyVaultStage ClientID/Key as stored in KeePass.  These values should probably end up in Key Vault itself instead.  Backlog task.
$AADClientID = "10d98b12-20c1-4532-9bac-6b04da814258"
$AADKey = "M2U4Ykp0AWwUm7uk4uo27cyfgqsSqrDcYs8i9kS+rB4="
...
{
    $Found = $KV.AccessPolicies[$Count].DisplayName -like "AppSuiteKeyVaultStage*"
    If ($Found)
    {
...
If ($KV.EnabledForDiskEncryption)
{
    ...
    # Grant AppSuiteKeyVaultStage access with full control to manage Key Vault.
