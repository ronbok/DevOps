<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to retrieve and display an existing tenant
...
.NOTES 
    ...
    Requires   : Azure Subscription needs to be setup in advance and you must know the name.
               ...
               : User must have sufficient privledges read the an existing tenant manifest table.
               :
	...
	Updated    : 04/14/2016 v1.1 Updated for tenant manifest schema changes.
    ...
    Get-Help ./List-Tenant.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER TenantDomain
    Example: "capecodhealth.org"
.EXAMPLE
    ./List-Tenant.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Subscription_Common" -AzureSAName "sassubcom1" -TableName "TMDevelopment" -TenantDomain "capecodhealth.org" -Verbose
.EXAMPLE
    ./List-Tenant.ps1 "AppSuite" "East US 2" "Subscription_Common" "sassubcom1" "TMDevelopment" "capecodhealth.org" -Verbose
.INPUTS
    ...
    The [TableName] parameter is the name of Tenant Manifest Table from which to list the tenant.
    
    The [TenantDomain] parameter is the name of Tenant.
.OUTPUTS
...
Param(
    ...
    [string]$TableName,
    [Parameter(Mandatory=$True, Position=5, HelpMessage='The Steward AppSuite Tenant (FQDN) Domain Name.')]
    [string]$TenantDomain
    )
...
If ($Table -ne $null)
{
    ...
    # If we don't already have a $TenantDomain row in our $TableName we fail with a warning.
    If ($Entity -ne $null)
    {
