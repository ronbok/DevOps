<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to create a new Azure Resource Manager StorageAccount using an
	Azure AD credential with proper RBAC permissions to create such a group.  This PowerShell Script makes it easier
    to create an Azure Resource Manager StorageAccount instead of using the Management Portal interactively.
.DESCRIPTION 
    This creates one or more Storage Accounts.  The StorageAccount is tagged based upon the values entered for the
	...
	Each of the string values, AzureRGName, AzureSAName, and AzureSAType must be paired equally or the script will fail and
	exit.
.NOTES 
    File Name  : Create-AzureRMStorageAccount.ps1
               :
    ...
    Requires   : Azure Subscription needs to be setup in advance and you must know the name.
               ...
               :
    Optional   : This can be called from a .bat file as desired to create one or more Azure StorageAccount(s).
			   : An example is included which demonstrates how to provide input for creating multiple Storage Accounts as
			   : a comma separated list including various Storage Account Types.
               :
    Created    : 03/08/2016 v1.0
	Updated	   : 03/15/2016 v1.1
	Updated    : MM/DD/2016:
    ...
    Get-Help ./Create-AzureRMStorageAccount.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureSAType
    Example: "Standard_GRS,Standard_GRS,Premium_LRS"
.EXAMPLE
    ./Create-AzureRMStorageAccount.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Development_Development,Development_Common,Development_Common,Test_Test,Test_Common,Test_Common,Stage_Stage,Stage_Common,Stage_Common,Production_Common,Production_Common,Production_Common,Production_Common,Subscription_Common,Subscription_Common" -AzureSAName "sasdds,sasdcs,sasdcp,sastts,sastcs,sastcp,sassss,sasscs,sasscp,saspcs1,saspcs2,saspcp1,saspcp2,sassubcom1,sassubcom2" -AzureSAType "Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Premium_LRS,Standard_GRS,Standard_GRS" -Verbose
.EXAMPLE
    ./Create-AzureRMStorageAccount.ps1 "AppSuite" "East US 2" "Development_Development,Development_Common,Development_Common,Test_Test,Test_Common,Test_Common,Stage_Stage,Stage_Common,Stage_Common,Production_Common,Production_Common,Production_Common,Production_Common,Subscription_Common,Subscription_Common" "sasdds,sasdcs,sasdcp,sastts,sastcs,sastcp,sassss,sasscs,sasscp,saspcs1,saspcs2,saspcp1,saspcp2,sassubcom1,sassubcom2" "Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Premium_LRS,Standard_GRS,Standard_GRS"
.INPUTS
    The [Subscription] parameter is the name of the Azure subscription.

    The [AzureRegion] parameter is the name of the Azure Region/Location in which to place the Storage Account(s).

    The [AzureRGName] parameter is the name of the Azure Resource Group to place the Storage Account(s).

    The [AzureSAName] parameter is the name of the Azure Storage Account to create.

    The [AzureSAType] parameter is the Azure Storage Account Type desired.
.OUTPUTS
...
Param(
    ...
    [string]$AzureRegion,
    [Parameter(Mandatory=$True, Position=2, HelpMessage='The Azure Resource Group in which to place the Storage Account.')]
    [string]$AzureRGName,
    [Parameter(Mandatory=$True, Position=3, HelpMessage='The Azure Storage Account to create.')]
    [string]$AzureSAName,
    [Parameter(Mandatory=$True, Position=4, HelpMessage='The type of Azure Storage Account to create.')]
    [string]$AzureSAType
    )
...
[string]$AzureSAType = "Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Standard_GRS,Standard_GRS,Premium_LRS,Premium_LRS,Standard_GRS,Standard_GRS"
...
[string[]]$AzureSATypes = $AzureSAType.Split(",")
# [string[]]$AzureSAContainerNames = $AzureSAContainerName.Split(",")
...
If (($AzureRGNames.Count -eq $AzureSANames.Count) -and ($AzureRGNames.Count -eq $AzureSATypes.Count) -and ($AzureSANames.Count -eq $AzureSATypes.Count))
{
    While ($Count -le $AzureRGNames.Count-1)
    {
        ...
        If ([bool]$CheckSA.NameAvailable)
        {
            ...
            Try
            {
	            ...
	            # Console output with -Verbose only
                Write-Verbose -Message "[Start] Attempting to create Azure Storage Account: $($AzureSANames[$Count])."
                Write-Verbose -Message ""

                # Create the Storage Account
                $SA = New-AzureRmStorageAccount -ResourceGroupName $AzureRGNames[$Count] -AccountName $AzureSANames[$Count] -Location $AzureRegion -Type $AzureSATypes[$Count] -Tags @{Name="Environment";Value=$($AzureRGNames[$Count].Split('_')[0])}, @{Name="Tenant";Value=$($AzureRGNames[$Count].Split('_')[1])}

                # Console output with -Verbose only
                Write-Verbose -Message "[Finish] Created Azure Storage Account: $($AzureSANames[$Count])."
                Write-Verbose -Message ""
            ...
            Catch
            {
	            ...
	            Write-Verbose -Message "[Error:$PSScriptName] Attempt to create Azure Storage Account: $($AzureSANames[$Count]) failed."
                Write-Verbose -Message ""
        ...
        Else
        {
            ...
            # Console output with -Verbose only
            Write-Verbose -Message "[Error:$PSScriptName] Attempt to create Azure Storage Account: $($AzureSANames[$Count]) failed.  Duplicate name."
            Write-Verbose -Message ""
