...
.SYNOPSIS 
    ...
    to a Key Vault for creating, writing, and then reading the BEK from Bitlocker.  An Azure AD credential with
    proper RBAC permissions to alter such permissions in an existing vault is required.  The Azure AD application
    must be created manually, it's ClientID and Key values must be known in advance.  An existing Azure AD application
    already exists by the name AppSuiteBitlockStage and the following code uses these values of ClientID/Key from that
    specific application instance.
.DESCRIPTION 
    This grants Azure Key Vault permissions to an Azure AD Application previously registered.

.NOTES 
    File Name  : Grant-AppSuiteBitlockerStage.ps1
               :
    ...
    Get-Help ./Grant-AppSuiteBitlockerStage.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureRGName
    Example: "Stage_Common"
.PARAMETER AzureKeyVaultName
    Example: "sastcs"
.EXAMPLE
    ./Grant-AppSuiteBitlockerStage.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Stage_Common" -AzureKeyVaultName "sastcs" -Verbose
.EXAMPLE
    ./Grant-AppSuiteBitlockerStage.ps1 "AppSuite" "East US 2" "Stage_Common" "sastcs" -Verbose
.INPUTS
    ...
    The [AzureKeyVaultName] parameter is the name of the Azure Key Vault to update.
.OUTPUTS
...
[string]$AzureRGName = "Stage_Common"
[string]$AzureKeyVaultName = "sastcs"
...
# Azure AD AppSuiteBitlockerStage ClientID/Key as stored in KeePass.  These values should probably end up in Key Vault itself instead.  Backlog task.
$AADClientID = "032705f1-3021-4bb5-a19a-054711bd6d2e"
$AADKey = "XZAOxBYSA5lOJ11bsBOKsrf9e3tebcu165cGEOa01nE="
...
{
    $Found = $KV.AccessPolicies[$Count].DisplayName -like "AppSuiteBitlockerStage*"
    If ($Found)
    {
...
If ($KV.EnabledForDiskEncryption)
{
    ...
    # Grant AppSuiteBitlockerStage access with full control to manage Key Vault.
    Set-AzureRmKeyVaultAccessPolicy -VaultName $KV.VaultName -ServicePrincipalName $AADClientID -PermissionsToKeys all -PermissionsToSecrets all
