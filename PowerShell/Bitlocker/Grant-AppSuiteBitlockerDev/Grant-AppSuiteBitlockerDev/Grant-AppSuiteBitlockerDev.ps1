...
.SYNOPSIS 
    ...
    already exists by the name AppSuiteBitlockDev and the following code uses these values of ClientID/Key from that
    specific application instance.
.DESCRIPTION 
    This grants Azure Key Vault permissions to an Azure AD Application previously registered.

.NOTES 
    File Name  : Grant-AppSuiteBitlockerDev.ps1
               :
    ...
    Get-Help ./Grant-AppSuiteBitlockerDev.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureKeyVaultName
    Example: "sasdcs"
.EXAMPLE
    ./Grant-AppSuiteBitlockerDev.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Development_Common" -AzureKeyVaultName "sasdcs" -Verbose
.EXAMPLE
    ./Grant-AppSuiteBitlockerDev.ps1 "AppSuite" "East US 2" "Development_Common" "sasdcs" -Verbose
.INPUTS
...
# Azure AD AppSuiteBitlockerDev ClientID/Key as stored in KeePass.  These values should probably end up in Key Vault itself instead.  Backlog task.
...
{
    $Found = $KV.AccessPolicies[$Count].DisplayName -like "AppSuiteBitlockerDev*"
    If ($Found)
    {
...
If ($KV.EnabledForDiskEncryption)
{
    ...
    # Grant AppSuiteBitlockerDev access with full control to manage Key Vault.
