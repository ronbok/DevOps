...
.SYNOPSIS 
    ...
    already exists by the name AppSuiteBitlockProd and the following code uses these values of ClientID/Key from that
    specific application instance.
.DESCRIPTION 
    This grants Azure Key Vault permissions to an Azure AD Application previously registered.

.NOTES 
    File Name  : Grant-AppSuiteBitlockerProd.ps1
               :
    ...
    Get-Help ./Grant-AppSuiteBitlockerProd.ps1 [Null], [-Full], [-Detailed], [-Examples]
...
.PARAMETER AzureRGName
    Example: "Production_Common"
.PARAMETER AzureKeyVaultName
    Example: "saspcs"
.EXAMPLE
    ./Grant-AppSuiteBitlockerProd.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGName "Production_Common" -AzureKeyVaultName "saspcs" -Verbose
.EXAMPLE
    ./Grant-AppSuiteBitlockerProd.ps1 "AppSuite" "East US 2" "Production_Common" "saspcs" -Verbose
.INPUTS
...
[string]$AzureKeyVaultName = "saspcs"
...
# Azure AD AppSuiteBitlockerProd ClientID/Key as stored in KeePass.  These values should probably end up in Key Vault itself instead.  Backlog task.
$AADClientID = "b86f77b8-20bb-4da2-841b-894bcb8a9374"
$AADKey = "sv1zDrrEiz4pk2vGKVTZI+ObPHZml9dTzYrWOTFF5yg="
...
{
    $Found = $KV.AccessPolicies[$Count].DisplayName -like "AppSuiteBitlockerProd*"
    If ($Found)
    {
...
If ($KV.EnabledForDiskEncryption)
{
    ...
    # Grant AppSuiteBitlockerProd access with full control to manage Key Vault.
