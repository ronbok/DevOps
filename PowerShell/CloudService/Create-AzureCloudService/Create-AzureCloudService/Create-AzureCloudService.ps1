<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to create one or more Cloud Service(s) using an
	Azure AD credential.  This PowerShell Script makes it easier to create one or more Cloud Service(s) instead
...
.NOTES 
    File Name  : Create-AzureCloudService.ps1
    ...
    Requires   : Azure Subscription needs to be setup in advance and you must know the name.
               ...
               :
    Optional   : This can be called from a .bat file as desired to create one or more Cloud Service(s).
			   : An example is included which demonstrates how to provide input to create one or more Cloud Service(s).
               :
    Created    : 03/31/2016
...
.EXAMPLE
    ./Create-AzureCloudService.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureCSName "sascsdc,sascstc,sascssc,sascspc"
.EXAMPLE
    ./Create-AzureCloudService.ps1 "AppSuite" "East US 2" "sascsdc,sascstc,sascssc,sascspc"
.INPUTS
...
While ($Count -le $AzureCSNames.Count-1)
    {
        ...
        {
            #region Create new Cloud Service in $AzureRegion if it doesn't exist.
            Try
            {
                $Error.Clear()
                $CloudServiceDescription = "Cloud Service for $($AzureCSNames[$Count]) in $Subscription in $AzureRegion."
    
                Write-Verbose -Message "[Start] Trying to create new Azure Cloud Service $($AzureCSNames[$Count]) in $Subscription."
     
                $CloudService = Get-AzureService -ServiceName $($AzureCSNames[$Count]) -ErrorAction Stop

                Write-Verbose -Message "[Finish] Azure Cloud Service $($AzureCSNames[$Count]) in $Subscription already exists."
            }
            Catch 
            {
                # Console output
                ...
                $AzureService = New-AzureService -ServiceName $($AzureCSNames[$Count]) -Location $AzureRegion -Description $CloudServiceDescription

                Write-Verbose -Message "[Finish] Successfully created new Azure Cloud Service $($AzureCSNames[$Count]) in $Subscription."
            }
        ...
        {
            # Console output
            Throw "Azure Cloud Service Name, $($AzureCSNames[$Count]) in $AzureRegion is not available! Azure Cloud Service Names must be UNIQUE worldwide. Aborting..."
        }
