<# 
.SYNOPSIS 
    This PowerShell script is intended to be executed remotely to remove one or more Azure Sql Databases using an
	Azure AD credential with proper RBAC permissions to create such a group.  This PowerShell Script makes it easier
    to remove one or more Azure Sql Databases instead of using the Management Portal interactively.
.DESCRIPTION 
    This removes one or more Azure Sql Databases on one or more Azure Sql Servers, but requires that they be in the EXACT
    same Resource Group.  NOTE: This PowerShell script does NOT support multiple Resource Groups at this time.
.NOTES 
    File Name  : Remove-AzureRmSqlDatabase.ps1
               :
    ...
    Requires   : Azure Subscription needs to be setup in advance and you must know the name.
               ...
               :
    Optional   : This can be called from a .bat file as desired to remove one or more Azure Sql Databases.
			   : An example is included which demonstrates how to provide input to remove one or more Azure Sql Databases.
               :
    Created    : 03/22/2016
	Updated	   : 03/22/2016 v1.0
...
.PARAMETER AzureDBName
    Example: "TestDb1"
.EXAMPLE
    ./Remove-AzureRmSqlDatabase.ps1 -Subscription "AppSuite" -AzureRegion "East US 2" -AzureRGEnvironmentName "Subscription" -AzureRGTenantName "Common" -AzureSQLServerName "sassubscription1" -AzureDBName "AuthTokenStore" -Verbose
.EXAMPLE
    ./Remove-AzureRmSqlDatabase.ps1 "AppSuite" "East US 2" "Subscription" "Common" "sassubscription1" "AuthTokenStore" -Verbose
.INPUTS
...
Param(
    ...
    [Parameter(Mandatory=$True, Position=4, HelpMessage='The name of the Azure Sql Database Server from which to remove the database.')]
    [String]$AzureSQLServerName,
    [Parameter(Mandatory=$True, Position=5, HelpMessage='The name of the Azure Sql Database to remove.')]
    [String]$AzureDBName
    )
...
If ($AzureSQLServerNames.Count -eq $AzureDBNames.Count)
{
    ...
    While ($Count -le $AzureSQLServerNames.Count-1)
    {
        ...
        If (![string]::IsNullOrEmpty($CheckSQL))
        {
            ...
            # Console output with -Verbose only
            Write-Verbose -Message "[Start] Removing database $($AzureDBNames[$Count]) on $($AzureSQLServerNames[$Count])."
            Write-Verbose -Message ""
            ...
            Remove-AzureRmSqlDatabase -ServerName $($AzureSQLServerNames[$Count]) -DatabaseName $($AzureDBNames[$Count]) -ResourceGroupName $($AzureRGEnvironmentName + "_" + $AzureRGTenantName) -Force
        
            # Delete the Audit Log
            Get-AzureStorageTable -Context $SaCLog | Where {$_.Name -like "SQLDBAuditLogs" + $($AzureDBNames[$Count]) +"*"} | Remove-AzureStorageTable -Force
                            
            # Console output with -Verbose only
            Write-Verbose -Message "[Finish] Removed database $($AzureDBNames[$Count]) on $($AzureSQLServerNames[$Count])."
            Write-Verbose -Message ""
